...
export interface BusinessRow { id: string; name: string; slug: string; owner_name: string; plan: string; expiry_date: string; status: string; mrr: number; override_mrr: number | null; signup_price: number | null; max_users: number; verticals: string[]; created_at: string; }
export interface BusinessInsert { name: string; slug: string; owner_name: string; plan?: string; expiry_date: string; status?: string; mrr?: number; override_mrr?: number | null; signup_price?: number | null; max_users?: number; verticals?: string[]; }
