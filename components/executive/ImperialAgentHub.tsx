...
export default function ImperialAgentHub() {
    ...
    const agentsData = useMemo(() => {
        ...
        return allAgentsCombined.map(dbA => {
            ...
            return {
                ...
                logs: specificLogs.length > 0 ? specificLogs.map(l => l.description) : [],
    ...
    return (
        <div className="space-y-8 animate-[fadeIn_0.5s_ease]">
                ...
                <div className="flex gap-4">
                    {isSovereign && (
                        <button 
                            ...
                            className="p-4 bg-slate-100 text-slate-600 rounded-2xl hover:bg-rose-50 hover:text-rose-600 transition-all"
                    ...
                    <AnimatePresence mode="wait">
                        {selectedAgent ? (
                            ...
                            <div className="bg-white rounded-[3rem] p-10 border border-gray-100 flex flex-col items-center justify-center text-center h-full min-h-[600px] opacity-40">
                                ...
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Ajan seçin</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
            ...
            <AnimatePresence>
                {showNewAgentModal && <NewAgentModal onClose={() => setShowNewAgentModal(false)} onSave={handleSaveNewAgent} />}
                {showTokenModal && <TokenPurchaseModal onClose={() => setShowTokenModal(false)} onSuccess={() => { setShowTokenModal(false); fetchAgents(); }} businessId={currentBusiness?.id} />}
                {showSystemSettings && <SystemSettingsModal onClose={() => setShowSystemSettings(false)} />}
            </AnimatePresence>
        </div>
    );
}
...
function TokenPurchaseModal({ onClose, onSuccess, businessId }: { onClose: () => void, onSuccess: () => void, businessId?: string }) {
    const packages = [
        { id: 'p1', name: 'Lejyoner', tokens: 50, price: '₺499' },
        { id: 'p2', name: 'Senatör', tokens: 250, price: '₺1.999', popular: true },
        { id: 'p3', name: 'İmparator', tokens: 1000, price: '₺6.999' }
    ];
    ...
    useEffect(() => {
        if (activeTab === 'history' && businessId) {
            supabase.from('ai_token_logs').select('*').eq('business_id', businessId).order('created_at', { ascending: false }).then(r => setTokenLogs(r.data || []));
        }
    ...
    const handlePurchase = async (pkg: any) => {
        ...
        try {
            const { error } = await supabase.rpc('add_ai_tokens', { p_business_id: businessId, p_amount: pkg.tokens, p_reason: `Paket: ${pkg.name}` });
            if (error) throw error;
            alert(`${pkg.tokens} Kredi yüklendi!`);
            onSuccess();
        } catch (err: any) {
    ...
    return (
        <div className="fixed inset-0 bg-indigo-950/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[3rem] p-12 max-w-4xl w-full shadow-2xl overflow-hidden">
                <div className="flex justify-between items-center mb-12">
                    <h3 className="text-2xl font-black uppercase text-indigo-950">Imperial Market</h3>
                    <div className="flex gap-2">
                        <button onClick={() => setActiveTab('buy')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase ${activeTab === 'buy' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>Market</button>
                        <button onClick={() => setActiveTab('history')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase ${activeTab === 'history' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>Geçmiş</button>
                        <button onClick={onClose} className="ml-4 text-slate-400"><X size={24} /></button>
                    </div>
                </div>
                ...
                {activeTab === 'buy' ? (
                    <div className="grid grid-cols-3 gap-6">
                        {packages.map(pkg => (
                            <div key={pkg.id} className="p-8 border-2 border-slate-50 rounded-[2rem] flex flex-col justify-between hover:border-indigo-600 transition-all">
                                <div>
                                    <h4 className="text-xs font-black uppercase text-slate-400 mb-2">{pkg.name}</h4>
                                    <p className="text-4xl font-black text-indigo-950">{pkg.tokens} <span className="text-xs opacity-30">Kredi</span></p>
                                </div>
                                <button onClick={() => handlePurchase(pkg)} disabled={isProcessing} className="mt-8 w-full py-4 bg-indigo-600 text-white rounded-xl font-black text-xs">{pkg.price} AL</button>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="max-h-[400px] overflow-y-auto">
                        <table className="w-full text-left">
                            <thead className="sticky top-0 bg-white"><tr className="border-b"><th className="py-4 text-[10px] font-black text-slate-400 uppercase">Tarih</th><th className="py-4 text-[10px] font-black text-slate-400 uppercase">İşlem</th><th className="py-4 text-right text-[10px] font-black text-slate-400 uppercase">Miktar</th></tr></thead>
                            <tbody>
                                {tokenLogs.map((log, i) => (
                                    <tr key={i} className="border-b last:border-0"><td className="py-4 text-xs font-bold text-slate-500">{new Date(log.created_at).toLocaleDateString('tr-TR')}</td><td className="py-4 text-xs font-black text-indigo-950 uppercase">{log.reason}</td><td className={`py-4 text-right font-black ${log.amount > 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{log.amount > 0 ? '+' : ''}{log.amount}</td></tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
...
function SystemSettingsModal({ onClose }: { onClose: () => void }) {
    ...
    useEffect(() => {
        supabase.from('system_config').select('value').eq('key', 'GEMINI_API_KEY').single().then(r => r.data && setApiKey(r.data.value));
    }, []);

    const handleSave = async () => {
        setIsSaving(true);
        try {
            await supabase.from('system_config').upsert({ key: 'GEMINI_API_KEY', value: apiKey });
            alert('API Anahtarı mühürlendi!');
            onClose();
        } catch (err: any) {
            alert('Hata: ' + err.message);
        } finally {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-[150] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-[3rem] p-12 max-w-xl w-full shadow-2xl">
                <div className="flex justify-between items-center mb-10">
                    <h3 className="text-2xl font-black uppercase text-rose-950">System Secrets</h3>
                    <button onClick={onClose} className="text-slate-400"><X size={24} /></button>
                </div>
                <div className="space-y-6">
                    <div>
                        <label className="text-[10px] font-black uppercase text-rose-400 mb-2 block">Gemini API Key</label>
                        <input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} className="w-full bg-slate-50 border p-4 rounded-2xl font-bold" placeholder="AIzaSy..." />
                    </div>
                    <button onClick={handleSave} disabled={isSaving} className="w-full py-5 bg-rose-600 text-white rounded-2xl font-black uppercase tracking-widest">{isSaving ? 'Kaydediliyor...' : 'MÜHÜRLE'}</button>
                </div>
            </motion.div>
        </div>
    );
...
function NewAgentModal({ onClose, onSave }: { onClose: () => void, onSave: (agent: any) => void }) {
    ...
    return (
        <div className="fixed inset-0 bg-indigo-950/40 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-[3rem] p-12 max-w-xl w-full shadow-2xl">
                <div className="flex justify-between items-center mb-10"><h3 className="text-2xl font-black uppercase">Yeni Ajan</h3><button onClick={onClose} className="text-slate-400"><X size={24} /></button></div>
                <div className="space-y-6">
                    <input value={name} onChange={e => setName(e.target.value)} placeholder="Ajan İsmi" className="w-full border p-4 rounded-2xl font-bold" />
                    <input value={role} onChange={e => setRole(e.target.value)} placeholder="Rolü" className="w-full border p-4 rounded-2xl font-bold" />
                    <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Talimatlar" className="w-full border p-4 rounded-2xl font-bold h-32" />
                    <button onClick={() => { onSave({ name, role, description: desc, mode }); onClose(); }} className="w-full py-5 bg-indigo-950 text-white rounded-2xl font-black uppercase">DEVREYE AL</button>
                </div>
            </motion.div>
        </div>
    );
